...
RUN_ARTIFACTS = [
    ...
    "predictions/prediction_manifest.sha256",
...
def main() -> None:
    ...
    if args.run_dir and not args.structural_only:
        run_dir = Path(args.run_dir)
        if not run_dir.is_absolute():
            run_dir = (root / run_dir).resolve()
        missing_run = [name for name in RUN_ARTIFACTS if not (run_dir / name).exists()]
        if missing_run:
            raise SystemExit(f"missing run artifacts in {run_dir}: {missing_run}")
    ...
    payload = {
        ...
        "status": "full_structural_manifest_created" if args.structural_only else "release_package_manifest_created",
