...
def test_enforce_security_policy_passes_on_clean_reports(tmp_path: Path):
    ...
    sbom.write_text(json.dumps({"components": [{"name": "pkg", "version": "1.0"}]}), encoding="utf-8")

    result, failures = enforce_security_policy(
        report_dir,
        repo_root=repo_root,
