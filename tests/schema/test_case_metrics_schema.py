...
def test_required_case_metric_columns_complete():
    assert "site_or_center" in REQUIRED_CASE_METRIC_COLUMNS
    assert "model_artifact_sha256" in REQUIRED_CASE_METRIC_COLUMNS
    assert "config_sha256" in REQUIRED_CASE_METRIC_COLUMNS
    assert "avd" in REQUIRED_CASE_METRIC_COLUMNS
...
def test_validate_case_metrics_columns_detects_missing():
    ...
    assert "site_or_center" in missing
    assert "model_artifact_sha256" in missing
